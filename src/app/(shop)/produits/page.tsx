  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 md:py-12">
      ...
      <div className="mb-8">
        ...
        <h1 className="text-3xl md:text-4xl font-black text-brand mb-1">Catalogue produits</h1>
        <p className="text-gray-500">
          <span className="font-bold text-primary-600">{products.length}</span> produits disponibles — triés par score tendance IA
        </p>
      </div>
      ...
      <div className="md:hidden mb-6">
        <form method="GET" className="space-y-3">
          <div className="relative">
            ...
            <input name="q" defaultValue={searchParams.q} placeholder="Rechercher un produit..." className="input-field pl-10" />
          </div>
          ...
          <button type="submit" className="btn-violet w-full text-sm py-3">Appliquer les filtres</button>
        </form>
      </div>
            ...
            <form method="GET" className="space-y-4">
              <div>
                <label className="text-xs font-bold uppercase tracking-wider text-gray-400 block mb-2">
                  <Search className="w-3.5 h-3.5 inline mr-1" /> Recherche
                </label>
                <div className="relative">
                  <Search className="absolute left-3 top-3 w-4 h-4 text-gray-400" />
                  <input name="q" defaultValue={searchParams.q} placeholder="Produit..." className="input-field pl-9 text-sm" />
                </div>
              </div>
              ...
              <div>
                <label className="text-xs font-bold uppercase tracking-wider text-gray-400 block mb-2">Catégorie</label>
        ...
        <div className="flex-1 min-w-0">
          ...
          {(searchParams.q || searchParams.categorie) && (
            <div className="flex items-center gap-2 mb-5 flex-wrap">
              ...
              {searchParams.categorie && (
                <span className="bg-primary-100 text-primary-700 text-xs font-bold px-3 py-1 rounded-full">
                  {categories.find(c => c.slug === searchParams.categorie)?.name}
                </span>
              )}
              <Link href="/produits" className="text-xs text-gray-400 hover:text-red-500 underline">Effacer tout</Link>
            </div>
          )}
          ...
          {products.length === 0 ? (
            <div className="text-center py-24">
              ...
              <p className="text-lg font-bold text-gray-700 mb-2">Aucun produit trouvé</p>
              <p className="text-gray-500 mb-6 text-sm">Essayez d'autres termes ou effacez les filtres</p>
